...
const MinimapElementShapes = ({
  ...
  if (isLinearElement(element)) {
    ...
    const fsCap = Math.max(element.height, 4);
    const fs = Math.min(
      Math.max((element.fontSize ?? 16) * 0.22, minStrokeScene * 4),
      fsCap,
    );
    inner = (
      <g opacity={opacity}>
        ...
        {element.text.trim() ? (
          <text
            ...
            style={{
              fontFamily:
                '"Virgil","Comic Sans MS","Apple Chancery",cursive,sans-serif',
              fontSize: fs,
              pointerEvents: "none",
              userSelect: "none",
            }}
